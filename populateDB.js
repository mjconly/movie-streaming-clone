    ...
    for (let j = 0; j < data.length; j++){
      ...
      for (let i = 0; i < cast.length; i++){
        ...
        if (i % 2 !== 0){
          ...
          else if(newActor.profile_pic !== currActor[1]){ //added this else to update actor pic if changed
            await newActor.updateOne({profile_pic: currActor[1]})
          }
      ...
      //adding this to append movies, not replace all
      await Movie.findOne({title: title})
        .then((movie) => {
          if (movie !== null){
            movie.updateOne({cast: actors_for_movie})
          }
          else{
            let newMovie = Movie({
              title,
